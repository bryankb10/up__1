...
void name(){
	...
	double phone;
	char gender[8];
	fptr=fopen("phonelist.txt","ab+");//ab+ gives us the ability of writing the function and add the second data in the existing one...
	if(fptr==NULL){
...
void modify(){
	...
	fclose(fptr);
	fclose(fptr1);
	fptr=fopen("phonelist.txt","w");
	fclose(fptr);
	fptr=fopen("phonelist.txt","a");
...
void password(void){
	char passwords[20]={"112233"};
	gotoxy(22,2);
	int j;
	int z;
	char name[40]="Authorized Personnel Only";
	z=strlen(name);
	for(j=0;j<=16;j++){
		Sleep(50);
		printf("\xDB");
